...
void NetworkManager::setupSocket()
{
    ...
    serverAddress.sin_addr.s_addr = m_ip_address == ANY_IP ? INADDR_ANY : inet_addr(m_ip_address.c_str());

    int bindResult = bind(m_server_socket, (struct sockaddr *)&serverAddress, sizeof(serverAddress));
    if (bindResult < 0)
    {
...
void NetworkManager::startListening(const std::function<void(char *, int)> &callback, bool async)
{
    ...
    if (async)
    {
        m_listener_thread = std::thread(&NetworkManager::startListenerLoop, this, callback);
...
void NetworkManager::startListenerLoop(const std::function<void(char *, int)> &callback)
{
    ...
    while (m_listening)
    {
        ...
        client_socket = accept(m_server_socket, (struct sockaddr *)&m_server_address, (socklen_t *)&addrlen);
        ...
        int bytes_received = read(client_socket, buffer, MAX_BUFFER_LENGHT_FOR_REQUESTS);
...
std::pair<std::string, int> NetworkManager::getRemoteAddress(int &remoteSocket)
{

    Logger &logger = Logger::getInstance();
    sockaddr_in remoteAddress;
    socklen_t remote_addr_len = sizeof(remoteAddress);
    int result = getpeername(remoteSocket, (struct sockaddr *)&remoteAddress, &remote_addr_len);
